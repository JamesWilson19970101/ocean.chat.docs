...
In the Ocean Chat architecture, clients frequently send `[0x0B] READ_RECEIPT` signals after pulling messages. To protect the underlying MongoDB and Redis from an IOPS "write storm," these cursor updates (`lastAckSeqId` and `lastReadSeqId`) are routed asynchronously to a dedicated NATS JetStream stream named `CURSOR_STATE`.
...
1.  **Client-Side Deduplication:** If the server loses the latest `lastAckSeqId`, the client's next sync request will fall back to an older cursor position stored in Redis/MongoDB. The server will deliver a broader range of messages, including ones the client has already seen.
