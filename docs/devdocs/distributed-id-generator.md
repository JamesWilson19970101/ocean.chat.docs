...
- **`ClientMsgId` (For Uniqueness):** A standard UUID generated by the _Client_ the moment the user clicks "Send". It is used exclusively by the server and client to silently deduplicate messages if network retries occur.
- **`SyncSeqId` (For Ordering & Sync):** A 64-bit integer generated by the _Server_ (`oceanchat-message`). It is strictly monotonically increasing **within the context of a single session (a specific P2P chat or a specific Group chat)**.
...
:::info Architecture Downgrade Notice: Not Required for Current Scale
**Ocean Chat's current design target is a user base of up to hundreds of thousands, with active sessions being even fewer.** At this scale, storing a separate `max_seq` record for each session directly in the database only consumes a few megabytes of data. This poses absolutely no pressure on modern databases and keeps the code logic much cleaner and simpler. Therefore, **in actual development, there is absolutely no need to introduce the complex Section-based merging storage concept.**
This section is retained solely to explore the ultimate optimization solutions adopted by national-level IMs like WeChat to save storage costs when facing **billion-level** massive sessions.
:::
