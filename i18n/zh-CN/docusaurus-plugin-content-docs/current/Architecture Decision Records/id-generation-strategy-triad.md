...
在一个支撑海量并发的分布式 IM 系统中生成 ID，面临着互相冲突的业务需求。我们需要为实体（用户、群组）生成去中心化的 ID 以防止单点瓶颈；我们需要在客户端网络重试时防止消息重复写入；最关键的是，我们需要高效地物理落盘消息，同时还要为客户端提供严格的单调递增序列，以便其执行消息防丢漏测（空洞检测）。
