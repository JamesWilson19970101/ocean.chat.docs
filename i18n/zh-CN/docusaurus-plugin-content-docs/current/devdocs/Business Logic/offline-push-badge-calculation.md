...
// 2. 截断滑动窗口，永远只保留最近的 100 条消息
ZREMRANGEBYRANK group:msg:G1001 0 -101
```
...
:::info O(1) 空间复杂度与兜底截断
正如前文所述，该 ZSET 永远只保留群内最近的 **100 条**消息（通过 `ZREMRANGEBYRANK` 截断）。如果用户的游标实在太老，超出了 ZSET 的保存区间，`ZCOUNT` 会返回 100，系统会直接将其作为 `100+` 的红点标志处理，使得内存和计算成本永远恒定。
:::
