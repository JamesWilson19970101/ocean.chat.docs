...
为了维持连接的存活，Ocean Chat 摒弃了僵化的定时心跳策略。这里需要严格区分两个不同层面的“心跳”，它们的周期与超时时间完全不同：

- **连接层保活（网关 ↔ 客户端）**：严格遵循 [Monkey Protocol 非对称心跳规范](../Monkey%20Protocol/monkey-asymmetric-heartbeat.md)。网关为每条连接维护 `lastActiveTime` 时间戳：连接空闲 **30 秒**时主动下发 `[0x03] PING` 探活；若空闲达到 **60 秒**（双端绝对超时）仍未收到任何数据，即判定为死链并强制切断。
- **状态层心跳（网关 → NATS → Redis）**：网关将本地仍然存活的连接，以低频批量事件的形式上报给 Presence 服务，仅用于维持 Redis 全局路由图谱的新鲜度，与连接层的探活与断连判定无关。

1. **业务包即心跳**：只要客户端发来了**任何**合法的上行数据包（无论是单纯的 `[0x03] PING` 还是聊天信令 `[0x05] MSG_UP`），网关都会立刻刷新该连接的 `lastActiveTime`。
2. **主动推送心跳**：网关每隔 3 分钟主动向 NATS 推送活跃设备的心跳 (`presence.conn.heartbeat`)。这种设计大幅减少了移动端在后台时发送无意义空包的频率。
3. **独立设备续期**：Presence 服务接收到心跳事件后，直接调用 Redis 的 `HEXPIRE` 指令，为 `user:routing:{userId}` 下对应的 `deviceId` 字段续期 300 秒（5分钟）的 TTL。（黄金比例策略：状态层心跳间隔 180s，Redis TTL 300s，容忍一次心跳事件丢失。）

:::info Redis 的 300 秒 TTL 不是探活超时
真正的死链早在空闲 **60 秒**时就已被网关的连接层保活机制切断，并即时发出 `presence.conn.offline` 事件、清理 Redis 路由。300 秒的字段 TTL 只是针对“offline 事件本身在极端情况下丢失”的最后一道兜底防线，防止 Redis 中残留幽灵路由。
:::
...
- **心跳超时 (Timeout)**：客户端进入深度休眠或彻底失联，既不回应 `PING` 也不再发送任何数据，导致连接空闲达到 **60 秒**绝对超时。网关的全局 Sweep 扫描将主动切断该僵尸连接。
