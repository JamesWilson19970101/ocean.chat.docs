...
这是 Monkey Protocol 在 SDK 层对可靠性至关重要的设计，并与统一的上行超时策略配合：

1. **入列**：任何带有 `ReqId` 且非 0 的业务请求（如 `[0x05] MSG_UP`），在发出时，连同其 Payload、发起时间与重试计数，存入内存的 `In-Flight Queue` 中。
2. **出列**：当收到带有相同 `ReqId` 的 `[0x06] MSG_UP_ACK` 响应时，将其从飞行队列移除。
3. **同连接超时重传**：每条在飞消息按 **5 秒 + 随机抖动** 等待 ACK；超时则带同一 `ClientMsgId` 重传，**最多 3 次**；仍失败则标记发送失败并通知 UI 显示红叹号。详见《如何保证在线消息的可靠性》。
4. **断线重放 (Replay)**：意外断线后，状态机在重连且 `AUTH_REQ` 成功认证后，将队列中尚未耗尽重试预算的遗留包按顺序静默重发（计为一次重传尝试），**不得**因“等待重连”而无限期保持发送中。
5. **幂等兜底**：重放 Payload 含原封不动的 `ClientMsgId`，服务端去重机制可安全过滤，绝不会在群里发两遍一模一样的内容。
...
3. **网关宏观轮询**：摒弃 per-socket 计时器，将十万级连接的心跳调度降维到单线程 O(N) 宏观扫描，解放了 CPU。
4. **飞行补偿闭环**：通过隐式心跳发现死链，触发指数重连，并在重连后对未确认信令执行受「5s+抖动 / 最多 3 次」约束的安全重放；耗尽后失败并展示红叹号。
