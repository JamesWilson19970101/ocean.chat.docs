...
乐观 UI 的基石在于 **`ClientMsgId` (强制使用基于时间排序的 UUIDv7)**：
...
为了支撑乐观 UI，客户端的本地数据库（Web 端为 IndexedDB，移动端为 SQLite）中，消息表（`Message`）必须具备以下三个关键字段：
...
2. SDK 生成 `ClientMsgId = "uuidv7-1234"`。
...
2. **更新 DB (状态失败)**：`send_status -> FAILED`（立即失败，不等待超时重传耗尽）。
3. **UI 响应**：“转圈圈”图标变成 **❗️ 红叹号**。用户点击可提示服务器返回的具体错误原因（`ExceptionAck.message`）。

---

## 4. 与“飞行中队列 (In-Flight Queue)”的深度协同

断网时的飞行队列自动重放，与乐观 UI 以及**统一的超时重传策略**配合使用。权威规则见《如何保证在线消息的可靠性》第四步：

- **ACK 等待：** 每次发出 / 重放 `MSG_UP` 后，等待 **5 秒 + 随机抖动**。
- **自动重传：** 超时未收到 `MSG_UP_ACK` 则带同一 `ClientMsgId` 重传，**最多 3 次**。
- **最终失败：** 重试耗尽后，`send_status -> FAILED`，UI 显示 **❗️ 红叹号**。
- **立即失败：** 收到业务 `EXCEPTION_ACK` 时立刻 `FAILED` + 红叹号，不等待重试耗尽。

当用户进入地铁（断网）并发了一条消息：

1. UI 执行乐观渲染，状态为 `SENDING`。
2. 底层网络发现死链，消息存入内存的 `In-Flight Queue`；ACK 等待定时器仍按「5s + 抖动」推进。
3. 网络恢复后，队列可自动重放未确认包（计为一次重传），收到 ACK 则 `SENDING -> SENT`。
4. **UI 表现：** 在重试预算内保持 `SENDING`（可展示“发送中 / 网络等待”）；**一旦重试满 3 次仍无 ACK，必须标记 `FAILED` 并显示红叹号**，不得无限期卡住。用户点击红叹号可手动重发（生成新的发送尝试，或复用原 `ClientMsgId` 由服务端幂等兜底）。
...
Monkey Protocol 倡导**“客户端作为 ID 的预先分配者”**（通过哈希或 UUIDv7 降维）：
...
3. 结合**飞行队列重放**、**「5s+抖动 / 最多 3 次」超时重传**与**服务端幂等去重**，在弱网下既可自动补发，又会在重试耗尽时明确失败（红叹号），且不会产生重复气泡或幽灵消息。
