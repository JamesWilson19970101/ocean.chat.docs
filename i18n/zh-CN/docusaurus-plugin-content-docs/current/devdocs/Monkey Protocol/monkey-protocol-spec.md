...
:::info 职责边界
在线 `MSG_NOTIFY` 的 200ms 折叠**只发生在服务端网关**。客户端 SDK 不必再为同一会话的唤醒信令实现 200ms 防抖；收到折叠后的通知即可触发 HTTP Sync。客户端侧仍可保留弱网场景下的“待同步游标合并”（例如 HTTP 拉取失败重试期间又收到新通知），那属于可靠性兜底，而非替代网关折叠。
:::
...
3. **用户交互**：根据 Payload 提供的 `errorCode` 及安全的 `message` 字段，在 UI 层面（如消息旁边的 **❗️ 红叹号**、Toast 提示等）给予用户反馈，并将对应乐观消息标记为 `FAILED`，而不会导致长连接崩溃。上行超时重传策略（5s+抖动、最多 3 次后同样显示红叹号）见《如何保证在线消息的可靠性》。
