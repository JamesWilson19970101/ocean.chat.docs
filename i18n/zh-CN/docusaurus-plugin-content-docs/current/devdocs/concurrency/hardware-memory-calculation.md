...
// 按理说内存应该不会成为瓶颈，这里就先不仔细计算了，稍后再补充这篇文章。原因如下：
DDR4（上一代/经济型服务器主流）：常见的 DDR4-3200，单根内存条的峰值带宽大约是 25.6 GB/s。
DDR5（当前新一代服务器/PC主流）：常见的 DDR5-4800，单根带宽约 38.4 GB/s。高端的 DDR5-6400，单根带宽可达 51.2 GB/s。
服务器架构的“多通道”：在企业级服务器（也就是承载 Ocean Chat 的物理机）中，主板和 CPU 绝不会只插一根内存条，也不会只用普通的双通道。现代服务器 CPU 采用的是多通道内存架构，这会让总带宽成倍飙升。
