...
* **100,000 个连接 × n KB = n × 100,000 KB ≈ n/10 GB** *(采用 1GB ≈ 1,000,000 KB 的工程近似估算，此宽估能在计算中天然提供约 4.8% 的安全冗余)*
...
* 单个在线用户占用：约 n Bytes。
* **100,000 在线用户 × n Bytes = n × 100,000 Bytes ≈ n/10 MB** *(同上采用工程近似估算)*。
...
3. **实时监控 IOPS 和内存命中率**：
   ...
   ```
   *(重点关注 `insert` 每秒插入批次数、`dirty` WT 脏数据比例 和 `res` 常驻物理内存大小)*
