...
本文档深入探讨了 Ocean Chat 背后的技术架构决策。为了支持 **千万级（10M+）并发连接**，系统的每一项设计都经过了严格优化，以确保确定性的性能、极高的资源效率和绝对的安全。
...
2.  **零同步阻塞**：为了支持千万级并发，登录接口必须在 50ms 内响应。NATS 从关键路径上消除了所有下游网络 I/O。
...
Mongoose `Document` 实例是包含复杂内部状态的重型类。在千万级并发规模下，它们会造成巨大的垃圾回收（GC）压力。
