...
<head>
  ...
  <meta property="og:description" content="深入解析 Ocean Chat 的核心架构决策，涵盖 NATS JetStream、事件驱动授权以及千万级并发处理模式。" />
  <link rel="canonical" href="https://jameswilson19970101.github.io/ocean.chat.docs/zh-CN/docs/devdocs/design-decisions-faq" />
</head>

# 架构决策与 FAQ

本文档深入解析了 Ocean Chat 背后的架构权衡。为了支持 **千万级（10M+）并发连接**，系统的每一项设计都经过了严格优化，以确保确定性的性能、极高的资源效率和绝对的系统安全性。

<Tabs>
  <TabItem value="nats" label="NATS 与事件驱动" default>

## JetStream 与事件驱动架构

### 为什么隔离 `AUTH_STATE` 和 `AUTH_EVENTS` 为独立的流？
在 NATS JetStream 中，Stream 是一种轻量级的逻辑结构。真正的开销在于消费时的网络带宽和 CPU 占用。我们强制执行严格隔离基于以下三个原因：

1.  **保留生命周期 (Retention)**：`AUTH_STATE`（撤销信号）是瞬态的，存在于极速的 **内存存储 (Memory Storage)** 中（约 15-30 分钟窗口）。`AUTH_EVENTS`（业务登录事件）需要长期审计，持久化在 **磁盘存储 (File Storage, SSD)** 中保留数天。
...
### 为什么通过 NATS 发布 `loggedIn` 事件而不是直接 RPC 调用？
1.  **极致解耦**：认证服务无需关注谁在消费这些数据（在线状态、审计、分析服务等）。
2.  **消除同步阻塞**：为了支持千万级并发，登录接口必须在 50ms 内响应。NATS 从关键路径上消除了所有下游网络 I/O。
3.  **流量削峰**：在流量激增期间（如全局推送导致大量用户同时登录），NATS 会将事件缓冲在磁盘上，允许下游服务以其最大安全速率拉取数据，避免系统崩溃。

### 为什么使用现代 NATS Consumer API 而不是 `js.subscribe()`？
1.  **临时有序消费者 (Ephemeral Ordered Consumers)**：在网络重连期间自动处理序列追踪和重置，无需繁杂的服务端管理。
2.  **异步迭代 (`consume()`)**：通过 `for await` 完美适配 Node.js 事件循环，确保消费逻辑非阻塞且易于维护。

</TabItem>
<TabItem value="performance" label="性能与安全">

## 性能与内存安全

### 为什么使用自定义的 `BoundedPublisherService` 而不是 `p-limit`？
标准的并发库如 `p-limit` 使用 **无界内部数组** 来存储任务积压。在极端负载下，这个数组会持续膨胀直至触发 **OOM (内存溢出)** 崩溃。我们的自定义解决方案提供了：
-   **有界背压 (Backpressure)**：强制执行 `maxQueueSize`。一旦达标，系统会主动丢弃非核心任务以维持整体稳定性。
-   **配额隔离**：确保关键的安全撤销指令比普通业务事件拥有更高的保障，防止流量高峰时的“队头阻塞”。

### 为什么在 Repository 层强制执行严格的 “POJO-only” 政策？
Mongoose `Document` 实例是包含复杂内部状态的重型类。在千万级系统中，它们会造成巨大的垃圾回收 (GC) 压力。
-   **读操作**：使用 `.lean()` 在驱动层完全绕过 Document 实例化。
-   **写操作**：在返回 Service 层之前立即调用 `.toObject()`。

### 为什么对内部 NATS 消息执行 “零信任” 校验？
即便在 VPC 内部，我们也视基础设施为不可信的数据源。
1.  **严格类型**：每条消息都会经过 `class-transformer` 和 `class-validator` 处理，防止非法数据（如 `Invalid Date`）污染 MongoDB。
2.  **ACK/NAK 路由**：校验失败的消息会立即执行 `ack()`（丢弃），防止无限重推死循环；而业务/数据库失败则执行 `nak()`（重试）。

</TabItem>
<TabItem value="caching" label="缓存与一致性">
...
<TabItem value="messaging" label="消息架构">
