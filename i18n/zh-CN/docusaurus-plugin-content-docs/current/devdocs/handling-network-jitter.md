...
1. **防抖与挂起：** 当 `MSG_NOTIFY` 到达时，提取其中的会话 ID 与 `SyncSeqId`。如果它大于该会话本地的 `MaxLocalSyncSeqId`，将其存入内存中**按会话维护**的“待同步 (Pending Sync)”变量中。（在线唤醒信令本身已由网关在 200ms 窗口内折叠；此处的合并仅服务于 HTTP 失败后的重试兜底。）
...
### 解决方案：超时重传 + HTTP 同步状态转正

1. UI 乐观地显示“转圈圈”的 `SENDING` 状态，并按 **5 秒 + 随机抖动** 等待 ACK；超时则自动重传，**最多 3 次**。
2. 若重试耗尽仍无 ACK，将本地标记为 `FAILED` 并显示 **❗️ 红叹号**（用户可手动重发）。
3. 若在重试预算内网络恢复：客户端重连后可飞行队列重放，或按场景一隐式发起 HTTP Sync。
4. HTTP / 重放路径若带回自己刚才那条消息：通过 `ClientMsgId` 匹配仍处于 `SENDING`（或用户尚未手动清理）的本地记录，覆写真实 `SyncSeqId` 并转为 `SENT`，红叹号 / 转圈圈消失。
