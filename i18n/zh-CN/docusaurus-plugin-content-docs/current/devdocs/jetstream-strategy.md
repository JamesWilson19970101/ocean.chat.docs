...
- 存储介质 (Storage): `StorageType.File` (磁盘文件/SSD)。
  - 原因: 虽然保留期短，但在十万级并发洪峰（如重大赛事直播时的大群互动）下，如果后端的微服务解析变慢，上行消息会瞬间在 NATS 积压。使用基于 SSD 的文件存储可以安全地把突发流量缓冲在磁盘中，彻底避免内存溢出 (OOM) 崩溃。
- 关键配置与设计细节:
...
- 生产者配置 (Producer: `oceanchat-message` 服务)
  - 发布逻辑: 完成好友/群组鉴权、内容合规审查、并分配会话级严格单调递增的 `SyncSeqId`（以单个单聊/群聊为递增维度）后，将最终业务消息体发布至此主题。
  - 配置详情与原因:
...
- 关键配置与设计细节:
  - max_age: 15分钟: 这是一个非常巧妙的滑动窗口设计。因为网关只需要拦截仍在有效生命周期内但被提前撤销的 Token。配置文件中 Access Token 自然寿命是 **15 分钟**，超过 15 分钟前的撤销事件就毫无保留价值了（因为 Token 自身校验就会过期失效）。(注意：这个值必须大于或等于 `jwt.accessExpiresIn` 配置的时间，当前二者对齐为 15 分钟)。
  - 消费者类型 (`Ephemeral + DeliverPolicy.All`): 网关没有配置 `durableName`，是一个临时消费者。它每次启动或断线重连时，都会带上 `DeliverPolicy.All` 策略，从头拉取当前流里所有存活的（即过去 15 分钟内）数据。这保证了网关在冷启动时能迅速重建完整的黑名单缓存，防止安全真空期。
...
- 消费者配置 (Consumer: `oceanchat-api-gateway` / `oceanchat-ws-gateway` 服务)
  ...
  - 配置详情与原因:
    ...
    - deliver_policy: `DeliverPolicy.All` (拉取全量历史):
      - 原因: 临时消费者一旦断开重连，中间的消息就会丢失。为了解决冷启动/网络闪断问题，网关每次连接都会要求 NATS 把流里现存的所有消息（过去15分钟内的所有撤销记录）全部重新发一遍。这就完美实现了网关内存黑名单的快速重建。
...
- 消费者配置 (Consumer: `oceanchat-pusher-offline` 工作单元)
  ...
  - 配置详情与原因:
    ...
    - **`max_deliver: 3` (最大投递次数)**:
      - **原因**: 防止死循环。如果某个用户的 DeviceToken 已经彻底失效（例如卸载了 App），导致苹果服务器持续返回 400 BadDeviceToken，连续 **3 次**投递依然失败后，NATS 消费者框架会将任务作为毒消息转入死信队列（DLQ），避免该任务永久卡在队列中空耗系统资源。
