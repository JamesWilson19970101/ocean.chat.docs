...
:::info 架构概览
整个平台采用分布式微服务架构，旨在支持千万级（1000万+）并发。它分为四个逻辑层，包含11个核心微服务和1条数据处理管道，确保职责清晰分离。
:::
...
- **身份认证**: 实现 **Zero-I/O Authentication**。它通过密码学方式验证 RS256 Access Token，并针对令牌黑名单（通过 NATS JetStream 事件同步）进行 `O(1)` 本地内存查找，完全消除了关键路径上的同步网络 I/O（如 Redis 查询），以支持千万级并发。对于无需认证的接口，则直接放行。
