...
发送方需要在内部维护一个“等待 ACK”的飞行队列，并严格执行统一的超时重传策略：

1. 发出 `[0x05] MSG_UP` 时，启动 **5 秒 + 随机抖动** 的 ACK 等待定时器（例如 `5000 + Math.random() * 1000` 毫秒），避免海量客户端在同一时刻齐步重传。
2. 若在窗口内收到匹配 `ReqId` 的 `[0x06] MSG_UP_ACK`，将该消息移出飞行队列，本地 `send_status` 转为 `SENT`。
3. 若定时器超时仍未收到 ACK：用**完全相同**的载荷（含原始 `ClientMsgId`）自动重传，并递增重试计数；每次重传重新启动「5 秒 + 随机抖动」定时器。
4. **最多重试 3 次**。仍未收到 ACK 时，将本地 `send_status` 置为 `FAILED`，UI 在消息气泡旁显示 **❗️ 红叹号**（用户可点击查看原因并手动重发）。
5. 若中途收到业务侧 `[0x0C] EXCEPTION_ACK`（敏感词、权限不足、限流等），**不必等满重试次数**，立即标记 `FAILED` 并显示红叹号。

:::info 与断网重连飞行队列的关系
连接层断线后的 `In-Flight Queue` 重放，仍受上述重试预算约束：重连后的自动重放计为一次重传尝试，**不得**无限期保持 `SENDING`。总策略始终是「5s+抖动超时重传，最多 3 次，然后失败」。
:::
